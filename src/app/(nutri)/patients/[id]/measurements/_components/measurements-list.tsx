...
export function MeasurementsList({ measurements, patientId, customTypes, customValues }: MeasurementsListProps) {
  ...
  return (
    <div className="space-y-4">
      {sortedMeasurements.map((measurement) => {
        // Find custom values for this measurement
          ...
          >
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-x-8 gap-y-2 sm:grid-cols-3 lg:grid-cols-6">
                ...
                {measurementCustomValues.map((customValue) => {
                  const customType = customTypes.find((t) => t.id === customValue.type_id);
                  if (!customType) return null;
                  return (
                    <div key={customValue.id}>
                      <p className="text-xs text-muted-foreground">{customType.name}</p>
                      <p className="font-medium">{customValue.value} {customType.unit}</p>
                    </div>
                  );
                })}
              </div>
